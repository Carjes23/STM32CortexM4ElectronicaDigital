...
GPIO_Handler_t handlerMenu = { 0 };
EXTI_Config_t handlerExtiMenu= { 0 };
...
void initSystem(void) {
	...
	handlerMenu.pGPIOx = GPIOC; //Se encuentra en el el GPIOC
	handlerMenu.GPIO_PinConfig_t.GPIO_PinNumber = PIN_13; //Es el pin numero 3
	handlerMenu.GPIO_PinConfig_t.GPIO_PinMode = GPIO_MODE_IN; //En este caso el pin se usara como entrada de datos
	handlerMenu.GPIO_PinConfig_t.GPIO_PinPuPdControl = GPIO_PUPDR_NOTHING; //No se usara ninguna resistencia pull-up o pull-down

	handlerExtiMenu.pGPIOHandler = &handlerMenu; // Se carga la config del pin al handler del EXTI
	handlerExtiMenu.edgeType = EXTERNAL_INTERRUPT_FALLING_EDGE; //Se define que se van a detectar flancos de bajada.

	extInt_Config(&handlerExtiMenu); //Se carga la configuracion usando la funcion de la libreria.
