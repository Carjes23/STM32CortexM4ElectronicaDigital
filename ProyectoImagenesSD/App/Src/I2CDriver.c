...
uint8_t i2c_readSingleRegister(I2C_Handler_t *ptrHandlerI2C, uint8_t regToRead){
	...
	/* 7. Generamos la condición de NoAck, para que el Master no responda y el slave solo envie 1 byte*/
	i2c_sendNoAck(ptrHandlerI2C);

	/* 8. Generamos la condición Stop, para que el slave se detenga despues de 1 byte */
...
void i2c_readMultipleRegisters(I2C_Handler_t *pHandlerI2C, uint8_t *rxData, uint8_t initialReg, uint8_t regsToRead){
	...
	i2c_sendSlaveAddressRW(pHandlerI2C, pHandlerI2C->slaveAddress, I2C_WRITE_DATA);
	...
	i2c_sendSlaveAddressRW(pHandlerI2C, pHandlerI2C->slaveAddress, I2C_READ_DATA);

	/* 6. Almacenar la información en el arreglo ingresado */
	for(uint8_t i = 0; i < (regsToRead); i++){
		rxData[i] = i2c_readDataByte(pHandlerI2C);
	}
