...
/**/
void spi_config(SPI_Handler_t2 ptrHandlerSPI){
...
/**/
void spi_transmit(SPI_Handler_t2 ptrHandlerSPI, uint8_t * ptrData, uint32_t dataSize){
...
/* Función para enviar y recibir un solo byte */
uint8_t spi_transmitByte(SPI_Handler_t2 ptrHandlerSPI, uint8_t data){
	...
	// Enviamos el dato al que apunta el puntero
	ptrHandlerSPI.ptrSPIx->DR = (uint8_t) (0xFF & data);

	// Esperamos a que se complete la transmisión
	while(!(ptrHandlerSPI.ptrSPIx->SR & SPI_SR_RXNE)){
		__NOP();
	}

	// Se retorna el dato
	return ptrHandlerSPI.ptrSPIx->DR;
}

/**/
void spi_receive(SPI_Handler_t2 ptrHandlerSPI, uint8_t * ptrData, uint32_t dataSize){
...
void spi_selectSlave(SPI_Handler_t2* ptrHandlerSPI){
	GPIO_WritePin(&ptrHandlerSPI->SPI_slavePin, RESET);
}

/* Seleccionamos el esclavo llevando el pin SS a Vcc */
void spi_unSelectSlave(SPI_Handler_t2* ptrHandlerSPI){
	GPIO_WritePin(&ptrHandlerSPI->SPI_slavePin, SET);
}

/* Función para modificar el baudrate de la comunicación */
void spi_setBaudrate(SPI_Handler_t2 ptrHandlerSPI, uint8_t baudrate){

	// Se revisa que el parámetro sea válido
	if(baudrate > 7){
		while(1);	// Parámetro inválido
	}
	else{

		// Se limpian los bits BR
		ptrHandlerSPI.ptrSPIx->CR1 &= ~(3 << SPI_CR1_BR_Pos);

		// Se escribe el nuevo valor
		ptrHandlerSPI.ptrSPIx->CR1 |= baudrate << SPI_CR1_BR_Pos;
	}
}
